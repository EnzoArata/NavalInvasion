...
bool SoundMgr::initWatercraftSounds(){
        ...
        for(unsigned int i=0; i<engine->entityMgr->entities.size();i++)//std::list<BaseEntity *>::const_iterator et = engine->entityMgr->entities[0]; et != engine->entityMgr->entities.size(); ++et)
            ...
            if (true){
                if (engine->entityMgr->entities[i]->audioID == 1){
...
void SoundMgr::attachSelectedNodeToSoundIndex(BaseEntity *ent, unsigned int index){
...
void SoundMgr::tick(double dtime){
	...
	syncListenerToCamera();
        ...
        //selection sound
		for (unsigned int i=0; i<engine->entityMgr->entities.size();i++){//(std::list<BaseEntity *>::const_iterator it = engine->entityMgr->entities.begin(); it != engine->entityMgr->entities.end(); ++it){
...
*/
bool SoundMgr::registerSelection(BaseEntity et, std::string filename){
    unsigned int sid;
    if (this->reserveAudio(filename, false, sid)){
                int lastIndex = -1;
                for (int i = 0; i < soundPerEnt; i++){
